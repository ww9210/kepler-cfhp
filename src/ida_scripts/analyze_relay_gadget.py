...
def is_indirect_call(head):
    ...
    if 'x86_indirect_thunk' in disasm:
        isIndirect=True
    return isIndirect
...
def is_parameter_relaying(interested_reg_value,tracked_register,target_reg=None):
    """
    bloom_num = 0
    ...
    if bloom_num >= len(tracked_register)+2:
        ...
        return False
    """
    if target_reg is not None and target_reg not in interested_reg_value:
        print target_reg
        return False
    if target_reg is not  None and interested_reg_value[target_reg] not in tracked_register:
        return False
    if interested_reg_value['rdi'] in tracked_register:
        return True
    return False
        ...
        for head in Heads(block.startEA, block.endEA):
            ...
            if is_mov_or_lea(mnem):
                ...
                if is_indirect_call(head):
                    ...
                    if is_parameter_relaying(interested_reg_value, tracked_register, target_reg):
            ...
            if res is not False:
                ...
                for key in reg_state:
                    if reg_state[key] == 'rdi':
                        print key,
                print ''
                relay_gadget.append([funcea, func_name, call_site, reg_state])

    print 'there are %d relay gadget' % len(relay_gadget)
    the_filename = "relay_gadget.txt"
    with open(the_filename, 'wb') as f:
           pickle.dump(relay_gadget, f)
