...
def analyze_xor(head,interested_reg_value):
	...
	try:
		...
		print 'failed in disassembly'
	return 
def is_target_reg_under_control(interested_reg_value,tracked_register,target_reg=None):
	if target_reg==None:
		return False
		...
		for head in Heads(block.startEA, block.endEA):
			...
			if is_mov_or_lea(mnem):
				...
				if is_indirect_call(head):
					...
					if is_target_reg_under_control(interested_reg_value, tracked_register, target_reg):
						num_indirect_call+=1
						res.append([True, head, interested_reg_value])
						if num_indirect_call >= 2 :
							return res
				...
				if is_indirect_jump(head):
					...
					if is_target_reg_under_control(interested_reg_value, tracked_register, target_reg):
						...
						#res.append([True, head, interested_reg_value])
							...
							#return res
						pass
		if seen_call >= 2 and seen_call > num_indirect_call:
			break
	...
	for segea in Segments():
		for funcea in Functions(segea, SegEnd(segea)):
			...
			if 'sub_' in func_name:
				continue
			res = is_forking_gadget(funcea)
			if res != False:
				#first_call_site = res[0][1]
				#first_reg_state = res[0][2]
				#second_call_site = res[1]
				forking_site_num = len(res)
				print '='*80
				print 'forking site %d'%(forking_site_num)
				print hex(funcea)
				print func_name
				for x in res:
					call_site=x[1]
					...
					for key in reg_state:
						if reg_state[key]=='rdi':
							print key,
					print ''
				fork_gadget.append([funcea, func_name, res])

	print 'there are %d forking gadget'%(len(fork_gadget))
	the_filename="fork_gadget.txt"
	with open(the_filename, 'wb') as f:
   		pickle.dump(fork_gadget, f)
